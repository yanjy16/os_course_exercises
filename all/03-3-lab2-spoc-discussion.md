...
1. X86有几个特权级？
+ 有四个特权级：内核级（最高）（level0）、（level1）、（level2）、应用级（level3），操作系统设计一般只需要两级（内核、用户）

2. 不同特权级有什么区别？
+ 特权指令只能在ring0执行，一条指令在不同的特权级下能访问的数据、执行后的效果不同

3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。
+ 访问门时，CPL<=DPL(门),且CPPL>=DPL(段)
...
+ 访问数据时，需要检查当前特权级是否等于或高于被访问的数据的DPL
+ 通过段寄存器访问数据段，检查CPL、RPL=DPL
+ 控制转移，只允许低特权级代码调用高特权级代码
+ 中断或异常，检查当前门特权级和目标代码特权级

2. 在什么情况下会出现特权级切换？
+ 低特权级代码调用高特权级代码时，或中断和异常

3. int指令在ring0和ring3的执行行为有什么不同？
+ int指令在ring0多进行了SS:ESP的压栈，进行了栈切换

4. 如何利用int和iret指令完成不同特权级的切换？
+ 构造需要的栈

5. TSS和Task Register的作用是什么？
+ TSS任务状态段：保存不同特权级所用到的信息，关键会用到其中的堆栈信息
+ Task Register：缓存全局描述符表GDT中TSS描述符信息。可以直接通过寄存器找到TSS的位置。
...
+ 由寄存器中的信息找到GDT里的表项，由描述符中的Base offset加上EIP得到线性地址，如果没有页机制，线性地址即物理地址，否则需要进一步转换
+ 是2次到3次？？？

2. 描述X86-32的MMU地址转换过程；
+ 由段寄存器中的信息找到GDT里的表项，由描述符中的Base offset加上EIP得到线性地址，如果没有页机制，线性地址即物理地址，否则需要进一步转换
...
+ 分配页，作为页目录表
+ 清除已分配的page
+ 建立地址映射：虚拟地址C0000000-F8000000 -> 物理地址00000000-38000000
+ 建立对等映射：虚拟地址00000000-00100000 -> 物理地址00000000-00100000
+ 使能CR0的第31位置一
+ 更新GDT
+ 取消对等映射
